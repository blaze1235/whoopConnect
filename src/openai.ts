...
- If some values are missing, calmly say that data is unavailable and explain the rest.`;
...
Write the entire answer only in ${languageName}. Keep it warm, calm, and simple. Do not use markdown headers, just short paragraphs.`;
}
...
/**
 * Generates the explanation. Uses the configured AI model when available,
 * otherwise falls back to the built-in free rule-based explainer. If the AI
 * call fails, also falls back so the user still gets a useful message.
 */
export async function generateExplanation(
  ...
  try {
    ...
    console.error(
      "AI request failed, using built-in explainer:",
...
// ---------------------------------------------------------------------------
// Built-in free explainer (no API needed). Rule-based, warm, non-medical.
// ---------------------------------------------------------------------------
...
function strainLevel(strain: number | null): Level {
  ...
  if (strain >= 14) return "low"; // high load -> needs care
  if (strain >= 8) return "medium";
  return "good"; // light day
}
...
function formatWorkoutListLocal(
  data: NormalizedWhoopData,
  language: Language
): string {
  return data.workouts
...
const RU: Texts = {
  greeting: (recovery) =>
    recovery !== null
      ? `Доброе утро. Сегодня восстановление: ${recovery}%.`
      : "Доброе утро. Вот сводка по вашим данным.",
  recovery: {
    good: "Организм хорошо восстановился. Сегодня можно жить в обычном ритме.",
    medium:
      "Восстановление среднее. Организм немного устал, лучше не перегружаться.",
  ...
  sleep: (level, hours) => {
    ...
    switch (level) {
      case "good":
        return `Сон был хороший.${hoursPart} Организм успел отдохнуть.`;
      case "medium":
        return `Сон был нормальный, но не полный.${hoursPart} Сегодня можно лечь немного раньше.`;
      case "low":
        return `Сна было мало.${hoursPart} Постарайтесь сегодня лечь пораньше и отдохнуть днём, если получится.`;
      default:
        return "Данные о сне пока недоступны.";
    }
  },
  strain: {
    good: "Вчера нагрузка была лёгкой. Сегодня можно спокойно погулять или сделать лёгкие дела.",
    medium:
      "Вчера нагрузка была средней. Сегодня можно делать обычные дела и прогулку.",
  ...
  advice: {
    ...
    medium:
      "Совет на сегодня: пейте воду, не перегружайтесь и лягте спать пораньше.",
    low: "Совет на сегодня: больше отдыхайте, пейте воду, избегайте тяжёлых дел. Если почувствуете себя плохо, обратитесь к врачу.",
    unknown:
      "Совет на сегодня: пейте воду, двигайтесь понемногу и хорошо отдыхайте.",
  },
  missing:
    "Некоторые данные сегодня недоступны. Это нормально — часы могли ещё не синхронизироваться.",
};

const UZ: Texts = {
  greeting: (recovery) =>
    recovery !== null
      ? `Xayrli tong. Bugungi tiklanish: ${recovery}%.`
      : "Xayrli tong. Ma'lumotlaringiz bo'yicha qisqacha xulosa.",
  recovery: {
    good: "Organizm yaxshi tiklangan. Bugun odatdagidek yashash mumkin.",
    medium:
      "Tiklanish o'rtacha. Organizm biroz charchagan, o'zingizni ortiqcha zo'riqtirmang.",
  ...
  sleep: (level, hours) => {
    const hoursPart =
      hours !== null ? ` Siz taxminan ${hours} soat uxladingiz.` : "";
    switch (level) {
      case "good":
        return `Uyqu yaxshi bo'lgan.${hoursPart} Organizm dam olishga ulgurgan.`;
      case "medium":
        return `Uyqu yomon emas, lekin to'liq emas.${hoursPart} Bugun biroz ertaroq uxlash foydali bo'ladi.`;
      case "low":
        return `Uyqu kam bo'lgan.${hoursPart} Bugun ertaroq uxlashga harakat qiling va iloji bo'lsa kunduzi dam oling.`;
      default:
        return "Uyqu haqidagi ma'lumot hozircha mavjud emas.";
    }
  },
  strain: {
    good: "Kecha yuklama yengil bo'lgan. Bugun bemalol sayr qilish yoki yengil ishlarni qilish mumkin.",
    medium:
      "Kecha yuklama o'rtacha bo'lgan. Bugun oddiy ishlar va yurish mumkin.",
  ...
  advice: {
    ...
    medium:
      "Bugungi maslahat: suv iching, o'zingizni ortiqcha charchatmang va ertaroq uxlang.",
    ...
    unknown:
      "Bugungi maslahat: suv iching, ozgina harakat qiling va yaxshi dam oling.",
  },
  missing:
    "Bugun ba'zi ma'lumotlar mavjud emas. Bu odatiy hol — soat hali sinxronlashmagan bo'lishi mumkin.",
};
...
export function buildLocalExplanation(
  ...
  // Advice follows the weakest of recovery/strain so it stays cautious.
  const adviceLevel: Level =
    rLevel === "low" || stLevel === "low"
      ? "low"
      : rLevel === "medium" || stLevel === "medium"
        ? "medium"
        : rLevel === "unknown" && stLevel === "unknown"
          ? "unknown"
          : "good";
